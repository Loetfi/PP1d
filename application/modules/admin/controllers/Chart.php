...
class Chart extends CI_Controller {
    ...
    **/
    ...
    function getDiagramBatang(){
        ...
        $query = $this->db->query("SELECT * from view_grafik_count_status where tahun='".$tahun."' order by tglYm ASC")->result_array();
    ...
    function getDiagramPieTerusan(){
        ...
        $query = $this->db->query("
			...
			from view_report_all_unit a
			left join ppid_unit b on a.id_unit = b.id_unit
			WHERE a.id_unit is not null AND tglYm = '".$tglYm."'
    ...
    function getDiagramPieSumber(){
		...
		$tglYm = ((int)$tahun * 100) + (int)$bulan;
        $query = $this->db->query("
			select count(*) nilai, sumber 
			from ppid_permohonan 
			where date_format(from_unixtime(cdate),'%Y%m') = '".$tglYm."'
			group by sumber 
			")->result_array();

...
<!-- end --> 
    ...
    if ($this->session->userdata('ses_ppid_user_code') == '' || $this->session->userdata('ses_ppid_user_foto') == '')
        echo anchor('profil', '<i class="fa fa-pencil"></i> Buat Permohonan','class="btn btn-default btn-sm profil"');
    else 
        echo anchor('permohonan/add', '<i class="fa fa-pencil"></i> Buat Permohonan','class="btn btn-default btn-sm"');
    ?>
<br><br>
    ...
    <table class="table table-bordered table-striped table-hover" id="thisDataTable">
        ...
        <tbody>
            ...
            <tr class="<?php echo $classRow; ?>" style="<?php echo $rowColor; ?>">
                ...
                <td>
                    <a href="<?php echo site_url('permohonan/detail'); ?>/<?php echo $row['no_permohonan']; ?>" class="btn btn-default btn-sm thisForRead" thisCode="<?php echo $row['no_permohonan']; ?>">Detail</a>
                </td>
            </tr>
            <?php } ?>
        </tbody>
...
$(function(){
   $('a.profil').click(function(){
       alert('Lengkapi Terlebih Dahulu Biodata Anda');
   });
});
</script>